  ...
  return (
    <div className="mx-auto max-w-6xl">
      <div className="mb-6">
        ...
        <p className="mt-1.5 text-muted">{clients.length} active coaching clients.</p>
      </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-line text-left text-xs uppercase tracking-wide text-muted">
              ...
              <th className="px-4 py-3 font-semibold">Last Workout</th>
          ...
          <Link key={c.id} href={`/coach/clients/${c.id}`}>
            <GlassCard variant="solid" className="p-4">
              ...
              <div className="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-xs text-faint">
                <span className="tnum">Week {c.currentWeek}/{c.totalWeeks}</span>
                <span>Last: {c.lastWorkout}</span>
                <span>Check-in: {c.checkIn}</span>
                <span className="tnum">{c.completion}% completion</span>
              </div>
            </GlassCard>
          </Link>
        ))}
