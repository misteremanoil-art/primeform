...
const copy = {
  en: { viewJourney: "View full journey" },
  ro: { viewJourney: "Vezi tot parcursul" },
} as const;
    ...
    <GlassCard variant="solid" className="flex h-full flex-col overflow-hidden p-0">
      ...
      <div className="flex flex-1 flex-col p-6">
        ...
        <div className="mt-3 flex items-baseline gap-2">
          ...
          <span className="text-sm text-muted">{study.headline.label[lang]}</span>
        </div>
        <ul className="mt-4 space-y-1.5">
          {study.metrics.map((m) => (
            <li key={m.label.en} className="flex items-center justify-between text-sm">
